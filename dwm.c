...
#define RMASTER                 selmon->pertag->rmasters[selmon->pertag->curtag]
...
deck(Monitor *m) {
	...
	if (n > m->nmaster) {
		mw = m->nmaster ? m->ww * (RMASTER ? 1.0 - m->mfact : m->mfact) : 0;
	...
	for (i = my = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++) {
		if (i < m->nmaster) {
			...
			resize(c, RMASTER ? m->wx + m->ww - mw : m->wx, m->wy + my, mw - (2*c->bw), h - (2*c->bw), c->bw, False);
			my += HEIGHT(c);
		} else
			resize(c, RMASTER ? m->wx : m->wx + mw, m->wy, m->ww - mw - (2*c->bw), m->wh - (2*c->bw), c->bw, False);
  }
}
...
tile(Monitor *m)
{
	...
	if (n > m->nmaster)
		mw = m->nmaster
			? m->ww * (RMASTER ? 1.0 - m->mfact : m->mfact)
			: 0;
	else
		mw = m->ww;
	for (i = my = ty = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		if (i < m->nmaster) {
			...
			resize(c, RMASTER ? m->wx + m->ww - mw : m->wx,
			       m->wy + my, mw - (2*c->bw), h - (2*c->bw), bw, 0);
			if (my + HEIGHT(c) < m->wh)
				my += HEIGHT(c);
			mfacts -= c->cfact;
		} else {
			h = (m->wh - ty) * (c->cfact / sfacts);
			resize(c, RMASTER ? m->wx : m->wx + mw, m->wy + ty,
			       m->ww - mw - (2*c->bw), h - (2*c->bw), bw, 0);
			if (ty + HEIGHT(c) < m->wh)
				ty += HEIGHT(c);
			sfacts -= c->cfact;
		}
...
togglermaster(const Arg *arg)
{
  selmon->pertag->rmasters[selmon->pertag->curtag] = !selmon->pertag->rmasters[selmon->pertag->curtag];
	...
	selmon->pertag->mfacts[selmon->pertag->curtag] = 1.0 - selmon->pertag->mfacts[selmon->pertag->curtag];
...
getparentprocess(pid_t p)
{
	...
	if (fscanf(f, "%*u %*s %*c %u", &v) != 1)
		fputs("warning: problems reading proc stat file\n", stderr);
...
centeredfloatingmaster(Monitor *m)
{
	...
	if (i < m->nmaster) {
		...
		resize(c, m->wx + mx, m->wy + my, w - (2*c->bw),
		       mh - (2*c->bw), borderpx, 0);
		mx += WIDTH(c);
	} else {
		/* stack clients are stacked horizontally */
		w = (m->ww - tx) / (n - i);
		resize(c, m->wx + tx, m->wy, w - (2*c->bw),
		       m->wh - (2*c->bw), borderpx, 0);
		tx += WIDTH(c);
	}
}
...
bstack(Monitor *m) {
	...
	for (i = mx = 0, tx = m->wx, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++) {
		if (i < m->nmaster) {
			...
			resize(c, m->wx + mx, m->wy, w - (2 * c->bw), mh - (2 * c->bw), borderpx, 0);
			mx += WIDTH(c);
		} else {
			h = m->wh - mh;
			resize(c, tx, ty, tw - (2 * c->bw), h - (2 * c->bw), borderpx, 0);
			if (tw != m->ww)
				tx += WIDTH(c);
		}
	}
...
bstackhoriz(Monitor *m) {
	...
	for (i = mx = 0, tx = m->wx, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++) {
		if (i < m->nmaster) {
			...
			resize(c, tx, ty, m->ww - (2 * c->bw), th - (2 * c->bw), borderpx, 0);
			if (th != m->wh)
				ty += HEIGHT(c);
		}
	}
...
gaplessgrid(Monitor *m) {
	...
	for(i = 0, c = nexttiled(m->clients); c; i++, c = nexttiled(c->next)) {
		...
		cx = m->wx + cn*cw;
		cy = m->wy + rn*ch;
		resize(c, cx, cy, cw - 2 * c->bw, ch - 2 * c->bw, borderpx, False);
		rn++;
		if(rn >= rows) {
			rn = 0;
			cn++;
